...
class Candidate:
    def __init__(self, params, order=None, factor=1, model_idx=0):
        self.params = params
        self.order = order
        self.factor = factor
...
def sample_params(population, population_size, sigmas, resize_cfg, bounds, N_models, change_model_p, optimize_order):
    ...
    for pop in range(population_size):
        ...
        if resize_cfg["optimize"]:
            factor = np.clip(parent.factor + np.random.normal(0, resize_cfg["sigma"]), resize_cfg["min"], resize_cfg["max"])
        else:
        ...
        new_population.append(Candidate(new_parameters, order, factor, model_idx))
...
class SimpleProblem:
    ...
    def __call__(self, parameters, order, resize_factor, model_idx, indices):
        ...
        model_lib = []
        for i in indices:
            model_lib.append(self.model_lib[i])

        score = 0
        for model in model_lib:
            score_m = self.evaluator.run([model], pipeline)
            score += np.mean(score_m)

        score = score/len(model_lib)
        return score
...
#    pipeline_cfg = [cfg.augs] # if cfg.use_both_lighting else [cfg.augs]
    ...
    # Hyperparameters
    n_gen = 20 #cfg.optimization["n_gen"]
